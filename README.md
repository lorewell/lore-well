...
> 游戏类型：无行走、地点切换式回合制 RPG。玩家在不同大地点间传送，在各大地点内部通过可视化小地图自由移动，与 NPC 对话、拾取物品、触发回合制战斗。
...
- [x] localStorage 存档持久化（SAVE_VERSION = 3）
...
### 双层地图系统
- [x] 5 个大地点（晨曦村 / 幽暗森林 / 森林深处 / 古代神殿遗迹 / 废弃矿洞）
- [x] 每个大地点含 3–5 个子节点，通过 N/S/E/W 方向连接
- [x] BFS 算法自动计算节点坐标，SVG 绘制连接线，生成可视化小地图
- [x] 子节点节点点击或方向键（已移除界面按钮，通过点击节点移动）均可移动
- [x] 世界地图传送：通过子节点的 `exits` 属性进入相邻大地点

### 人物 / NPC 系统
- [x] 玩家角色数据（HP / MP / ATK / DEF / SPD / LV / EXP）
- [x] 6 个 NPC（隐秘瀑布 / 老板娘玛格 / 铁匠托尔 / 长老艾尔文 / 村长格雷 / 杂货商梅娜）
- [x] 分支对话树（每个 NPC 有多个对话节点，支持选项跳转）
- [x] 对话框占用底部同一区域，所有对话含"没事了"快速关闭选项
- [x] 客栈"休息"功能（通过对话触发 HP/MP 全额恢复）
- [x] HUD——顶部状态栏（角色名 / LV / HP 条 / MP 条 / EXP 条 / 金币 / 当前地点）
...
### 装备 & 任务系统
- [x] 装备穿脱（武器 / 护甲 / 饰品）+ 属性自动重算（含 maxHp / maxMp 加成）
...
- [x] 商店系统：购买 + 出售（按类型兜底出售价格）
- [x] 4 个任务（长老委托 / 森林威胁 / 补给短缺 / 铁匠委托）
- [x] 底部 HUD 三栏布局：左（可视化小地图）/ 中（地点信息 + 交互）/ 右（菜单切换）
- [x] 角色状态面板（属性、装备、EXP）
- [x] 移动端适配：`100dvh`、`env(safe-area-inset-*)` iOS 安全区、响应式左栏宽度

### 部署
- [x] 多阶段 Dockerfile（node:22-alpine 构建 + nginx:1.27-alpine 托管）
- [x] nginx SPA 路由回落 + 静态资源 1 年强缓存
- [x] GitHub Actions 手动触发：构建镜像推送 GHCR → SSH 部署到服务器
...
├── index.css                  # 全局样式 + Tailwind 导入（100dvh）
│
├── types/
│   └── index.ts               # 全局 TS 类型定义（含 SubLocation）
│
├── data/                      # 静态游戏数据
│   ├── characters.ts          # 玩家角色初始模板
│   ├── enemies.ts             # 敌人定义（6 种）
│   ├── items.ts               # 物品定义（13 种）
│   ├── locations.ts           # 地点定义（5 个大地点 + 各自子地图）
│   ├── npcs.ts                # NPC 定义（6 个，含分支对话树）
│   ├── quests.ts              # 任务定义（4 个）
│   └── shops.ts               # 商店定义（铁匠铺 + 客栈小卖部）
│
├── store/
│   └── gameStore.ts           # Zustand store（全游戏逻辑，SAVE_VERSION=3）
│
├── game/                      # Phaser 相关
...
└── components/                # 复用 UI 组件
    ├── HUD.tsx                # 顶部角色状态栏
    ├── LocationPanel.tsx      # 底部 HUD（三栏：小地图 + 地点信息 + 菜单）
    ├── DialogBox.tsx          # 对话框（与底部 HUD 同区域，替换显示）
    ├── CombatPanel.tsx        # 战斗面板（全屏覆盖）
    ├── InventoryPanel.tsx     # 背包面板（全屏遮罩）
    ├── QuestLog.tsx           # 任务日志（全屏遮罩）
    ├── ShopPanel.tsx          # 商店面板（全屏遮罩）
    └── PauseMenu.tsx          # 暂停菜单（全屏遮罩）
```
